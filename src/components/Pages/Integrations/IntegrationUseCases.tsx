...
type UseCaseItem = { title: string; description: string; cta: string };

const IntegrationUseCases = () => {
  const t = useTranslations('integrations-holded');
  const useCases = t.raw('useCases') as { label: string; items: UseCaseItem[] };
  const formCta = (t.raw('hero') as { cta: string }).cta;

  return (
    <section id="casos" className="px-landing py-14 lg:py-20 bg-white">
      <div className="max-w-[88.875rem] mx-auto">
        <p className="text-purple-accents text-body1 uppercase mb-10 tracking-widest">{useCases.label}</p>
