...
type Section = { heading?: string; items: string[]; numbered?: boolean };
type ServiceCard = {
  label: string;
  title: string;
  price?: string;
...
const OffMenuCard = ({ label, title, price, tagline, isDark, ctaText, sections, badge, link }: ServiceCard) => {
  ...
  const flipTo = (toBack: boolean) => {
    ...
    setFlipped(toBack);
    gsap.to(cardInnerRef.current, { rotationY: toBack ? 180 : 0, duration: 0.65, ease: 'power3.inOut' });
  };

  const handleClick = () => {
    if (link) {
      // @ts-expect-error — dynamic service link string
      router.push(link);
    } else if (!isDesktop) {
      flipTo(!flipped);
    }
  ...
  const btnCls = isDark ? 'bg-white text-[#3C3C3B] hover:opacity-75' : 'bg-[#3C3C3B] text-white hover:opacity-75';
  ...
  return (
    ...
    >
      <div ref={cardInnerRef} className="flip-card-inner h-full">
        <div className="flip-card-face h-full rounded-[1.5rem] p-10 lg:p-12 flex flex-col justify-between" style={{ backgroundColor: bg }}>
          <div className="flex flex-col gap-5">
            ...
            {price && <p className={`text-h2 font-light leading-none ${isDark ? 'text-white/30' : 'text-[#3C3C3B]/30'}`}>{price}</p>}
        ...
        <div className="flip-card-face flip-card-back h-full rounded-[1.5rem] p-8 lg:p-10 flex flex-col gap-5 overflow-hidden" style={{ backgroundColor: bg }}>
          ...
          <div className="flex flex-col gap-6 flex-1 overflow-hidden">
            {(sections ?? []).slice(0, 2).map((section, i) => (
              <div key={i} className="flex flex-col gap-2">
                {section.heading && (
                  <h3 className="text-smallTag text-purple-accents uppercase tracking-widest">{section.heading}</h3>
                )}
                <ul className="flex flex-col gap-1.5">
                  {section.items.slice(0, 5).map((item, j) => (
                    <li key={j} className={`text-sm leading-snug flex gap-2 ${bodyBackCls}`}>
...
const FaqItem = ({ question, answer, isOpen, onClick }: FaqItemType & { isOpen: boolean; onClick: () => void }) => (
  <div className="border-b border-dark-primary">
    <button onClick={onClick} className="w-full flex items-center justify-between gap-4 py-6 text-left cursor-pointer" aria-expanded={isOpen}>
...
const Services = () => {
  ...
  const seo = t.raw('pageSeo') as { services: { title: string; description: string } };
  const page = t.raw('servicesPage') as {
    hero: { suptitle: string; title: string; description: string };
    services: Array<{ number: string; title: string; tagline: string; sections: Section[] }>;
    engagement: { title: string; models: Array<{ type: string; subtitle: string; price?: string; description: string; items: string[] }> };
    faq: { title: string; items: FaqItemType[] };
  ...
  useGSAP(
    () => {
      ...
      ScrollTrigger.batch(Array.from(cards), {
        ...
        once: true,
        onEnter: (els) =>
          gsap.to(els, { autoAlpha: 1, y: 0, duration: 0.85, stagger: 0.14, ease: 'power3.out' }),
      });
    },
          ...
          <div>
            {page.faq.items.map((item, i) => (
              <FaqItem
                ...
                answer={item.answer}
                isOpen={activeFaq === i}
                onClick={() => setActiveFaq((prev) => (prev === i ? undefined : i))}
              />
            ))}
          </div>
        </div>
