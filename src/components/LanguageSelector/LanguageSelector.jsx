...
const LanguageSelector = () => {
  ...
  const invertedSource = Object.entries(sourceSlugs).reduce(
    (acc, [key, val]) => {
      acc[val] = key;
      return acc;
    },
        ...
        return (
          <div key={lang} className="flex items-center cursor-pointer">
            ...
            >
              {lang}
              <div
                className={`border-b-2 ${isActive ? 'border-[#7874f4]' : 'border-transparent'} w-fit mx-auto px-2 `}
              >
                {' '}
              </div>
            </button>
            {index < SUPPORTED_LANGS.length - 1 && (
