...
function randomSphere(n: number, seed: number) {
  let s = seed;
  const rand = () => {
...
type SpherePoint = ReturnType<typeof randomSphere>[0];
type SphereInfo = { cx: number; cy: number; r: number };
...
function drawDot(ctx: CanvasRenderingContext2D, cx: number, cy: number, size: number, opacity: number) {
...
function renderSphere(
  ...
  points: SpherePoint[],
  cx: number, cy: number,
  rotY: number, radius: number, dotSize: number,
  mouseX: number, mouseY: number, time: number
) {
  const perspective = 600;
  const mouseR = radius * 0.55;
  const isMouseOver = Math.hypot(mouseX - cx, mouseY - cy) < radius * 1.1;
  ...
  const projected = points.map((p) => {
    ...
    const edgeFactor = Math.min(1, Math.hypot(r.x, r.y) / radius);
...
function renderPentagon(
  ctx: CanvasRenderingContext2D,
  cx: number, cy: number, radius: number, rotZ: number,
  mouseX: number, mouseY: number, spheres: SphereInfo[]
) {
  const sides = 5;
  ...
  for (let s = 0; s < sides; s++) {
    ...
    for (let i = 0; i < segsPerSide; i++) {
      ...
      const px1 = x1 + t1 * (x2 - x1); const py1 = y1 + t1 * (y2 - y1);
      const px2 = x1 + t2 * (x2 - x1); const py2 = y1 + t2 * (y2 - y1);
      const pmx = (px1 + px2) / 2; const pmy = (py1 + py2) / 2;

      let overlap = 0;
      for (const sp of spheres) {
        const t = Math.max(0, 1 - Math.hypot(pmx - sp.cx, pmy - sp.cy) / sp.r);
...
function HeroScene({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      const largeR = Math.min(w * 0.4, h * 0.36);
      const smallR = largeR * 0.28;
      const pentR = largeR * 0.587;
      const largeCx = w * 0.6; const largeCy = h * 0.65;
      const smallCx = w * 0.84; const smallCy = h * 0.3;
      const pentCx = w * 0.74; const pentCy = h * 0.35;
