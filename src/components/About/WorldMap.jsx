...
const WorldMap = () => {
      ...
      function animate() {
        if (dragRef.current) {
          ...
          thetaRef.current = Math.max(
            -THETA_LIMIT,
            Math.min(THETA_LIMIT, thetaRef.current + velRef.current.theta)
          );
        }
        ...
        const pins = markerVecs.map((vec, i) => {
          ...
          return {
            el,
            lx: px + (dx / len) * 28 - pw / 2,
            ly: py + (dy / len) * 28 - ph / 2,
            pw,
            ph,
            visible: p.visible,
          };
        ...
        for (let iter = 0; iter < 5; iter++) {
          for (let i = 0; i < pins.length; i++) {
            for (let j = i + 1; j < pins.length; j++) {
              ...
              const acx = a.lx + a.pw / 2,
                acy = a.ly + a.ph / 2;
              const bcx = b.lx + b.pw / 2,
                bcy = b.ly + b.ph / 2;
  ...
  const onPointerMove = (e) => {
    ...
    phiRef.current =
      dragRef.current.phi + ((e.clientX - dragRef.current.x) / s) * Math.PI * 2;
    thetaRef.current = Math.max(
      -THETA_LIMIT,
      Math.min(
        THETA_LIMIT,
