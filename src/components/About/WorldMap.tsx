...
const MARKERS = [
  ...
  { role: 'Founders', city: 'Barcelona', lat: 41.7, lng: 2 },
  { role: 'Full Stack', city: 'Barcelona', lat: 41, lng: 2.5 },
...
function projectVec3(vec: [number, number, number], phi: number, theta: number) {
...
const WorldMap = () => {
  ...
  const dragRef = useRef<{ x: number; y: number; phi: number; theta: number; lastPhi: number; lastTheta: number } | undefined>(undefined);
  const velRef = useRef({ phi: 0, theta: 0 });
  const globeRef = useRef<ReturnType<typeof createGlobe> | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const raf0 = requestAnimationFrame(() => {
      ...
      function animate() {
        if (dragRef.current) {
          ...
          thetaRef.current = Math.max(-THETA_LIMIT, Math.min(THETA_LIMIT, thetaRef.current + velRef.current.theta));
        }
    ...
    return () => {
      ...
      if (globeRef.current) { globeRef.current.destroy(); globeRef.current = undefined; }
    };
  }, []);

  const onPointerDown = (e: React.PointerEvent) => {
    velRef.current = { phi: 0, theta: 0 };
    dragRef.current = { x: e.clientX, y: e.clientY, phi: phiRef.current, theta: thetaRef.current, lastPhi: phiRef.current, lastTheta: thetaRef.current };
    e.preventDefault();
  };
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    thetaRef.current = Math.max(-THETA_LIMIT, Math.min(THETA_LIMIT, dragRef.current.theta + ((e.clientY - dragRef.current.y) / s) * Math.PI));
  };

  const onPointerUp = () => { dragRef.current = undefined; };
  ...
  useGSAP(
    () => {
      ...
      ScrollTrigger.create({
        ...
        start: 'top 82%',
        once: true,
        onEnter: () => gsap.to(canvasRef.current, { opacity: 1, scale: 1, duration: 1.4, ease: 'power3.out' }),
      });
    },
    { scope: sectionRef }
  );
    ...
    <section className="world-map-section" ref={sectionRef}>
      <div className="world-map-globe-wrap" ref={wrapRef}>
        ...
        {MARKERS.map((marker, i) => (
          <div key={i} ref={(el) => { labelRefs.current[i] = el; }} className="world-map-pin">
