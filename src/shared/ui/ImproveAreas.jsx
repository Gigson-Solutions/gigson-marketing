...
const useFaqVisibility = (painPoints) => {
  ...
  const [selectedPainPoints, setSelectedPainPoints] =
    useState(defaultPainPoints);
  ...
  return (
    <div className={`flex flex-col md:flex-row flex-wrap ${boxClass}`}>
      {painPoints.map(({ type, name }, index) => {
        const isActive =
          (isDialog && selectedPainPoints.includes(type)) ||
          (!isDialog && selectedPainPoints.includes(type) && !initialChipState);
  ...
  return (
    <div>
      {faqs.map(({ question, answer, types }, faqIndex) => {
        const isVisible = types.some((type) =>
          selectedPainPoints.includes(type)
        );
...
const ResetFiltersButton = ({ onClick, text }) => {
  return (
    <span
      role="button"
      className="text-button text-[#A0A09F] cursor-pointer hover:opacity-80"
      onClick={onClick}
    >
      {text}
    </span>
...
const ImproveAreas = ({
  ...
  const hasMultipleFilters =
    selectedPainPoints.length > 0 && !initialChipsState;
      ...
      {dialogOpen && (
        <Dialog isOpen={dialogOpen} onClose={() => setDialogOpen(false)}>
          <div className="flex flex-col">
            <p className="text-subtitle text-dark-primary mb-4">
              {filtersText}
            </p>
            <div className="mb-20">
              <PainPointsChips
