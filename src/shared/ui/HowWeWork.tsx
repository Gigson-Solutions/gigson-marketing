...
type Method = { title: string; description: string };

const MethodCard = ({ title, description, isInnerMethod, isFirstCard }: Method & { isInnerMethod?: boolean; isFirstCard?: boolean }) => {
...
type Props = { title: string; methods: Method[] };

const HowWeWork = ({ title, methods }: Props) => {
        ...
        <div className="flex flex-col lg:flex-row flex-wrap gap-10">
          <div className="bg-purple-light flex flex-col items-center justify-center flex-8 lg:order-2">
            <div className="flex justify-center items-center w-full h-full p-6">
              <img className="!block lg:max-w-[50%]" src={logoSrc} alt="discover logo" />
            </div>
            {firstMethod?.map(({ title, description }, index) => (
              <MethodCard key={index} isInnerMethod title={title} description={description} />
            ))}
          </div>
          <div className="flex flex-col gap-y-6 flex-4 md:order-1">
            {methods.map(({ title, description }, index) => (
              <MethodCard key={index} title={title} description={description} isFirstCard={index === 0} />
            ))}
          </div>
        </div>
      </div>
